...
def rootAnalyse(bdt_model, bdt_taggers, dnn_model, dnn_taggers, dnn_scaler, data_files):
    ...
    for data_file in data_files:
        tfile = ROOT.TFile(data_file)
        tree = tfile.Get('dibjet')

        tfile_out = ROOT.TFile(data_file.replace('.root', 'scored_v2.root'), 'recreate')
        ...
        bdt_predict_arr_2 = np.recarray((1,), dtype=[('EEC_C2_1',float),('SPLIT12', float),('EEC_D2_1',float), ('TauWTA2TauWTA1', float), ('PlanarFlow', float), ('Sphericity', float), ('Aplanarity', float), ('nTracks', int)])
        
        dnn_predict_arr = np.recarray((1,), dtype=[('',type),('',type)])
        dnn_predict_arr_2 = np.recarray((1,), dtype=[('',type),('',type)])
        for e in range(entries):
            tree.GetEntry(e)
            # score for bdt/ dnn
            # have to stick the
            #for b in bdt_vars:
            bdt_predict_arr['EEC_C2_1'][0] = tree.jetTrim1_c2beta1
            bdt_predict_arr['EEC_D2_1'][0] = tree.jetTrim1_d2beta1
            bdt_predict_arr['Aplanarity'][0] = tree.jetTrim1_aplanarity
            bdt_predict_arr['SPLIT12'][0] = tree.jetTrim1_groosplit12
            bdt_predict_arr['TauWTA2TauWTA1'][0] = tree.jetTrim1_grootau21
            bdt_predict_arr['PlanarFlow'][0] = tree.jetTrim1_planarflow
            bdt_predict_arr['Sphericity'][0] = tree.jetTrim1_sphericity
            bdt_predict_arr['nTracks'][0] = tree.jetTrim1_ungrngtrk
            
            bdt_predict_arr_2['EEC_C2_1'][0] = tree.jetTrim2_c2beta1
            bdt_predict_arr_2['EEC_D2_1'][0] = tree.jetTrim2_d2beta1
            bdt_predict_arr_2['Aplanarity'][0] = tree.jetTrim2_aplanarity
            bdt_predict_arr_2['SPLIT12'][0] = tree.jetTrim2_groosplit12
            bdt_predict_arr_2['TauWTA2TauWTA1'][0] = tree.jetTrim2_grootau21
            bdt_predict_arr_2['PlanarFlow'][0] = tree.jetTrim2_planarflow
            bdt_predict_arr_2['Sphericity'][0] = tree.jetTrim2_sphericity
            bdt_predict_arr_2['nTracks'][0] = tree.jetTrim2_ungrngtrk
            j1_bdt_pred = bdt_model.predict_proba(bdt_predict_arr)
            j2_bdt_pred = bdt_model.predict_proba(bdt_predict_arr_2)
            jetTrim1_bdt[0] = j1_bdt_pred[0]
            jetTrim2_bdt[0] = j2_bdt_pred[0]

            means = {}
            std = {}
            for i, v in dnn_scaler.variables:
            ...
            dnn_predict_arr['split12'][0] = (tree.jetTrim1_groosplit12 - means['split12']) / std['split12']
            ...
            dnn_predict_arr_2['eec_c2_1'][0] = (tree.jetTrim1_c2beta1 - means['eec_c2_1']) / std['eec_c2_1']
            dnn_predict_arr_2['eec_d2_1'][0] = (tree.jetTrim1_d2beta1 - means['eec_d2_1']) / std['eec_d2_1']
            dnn_predict_arr_2['aplanarity'][0] = (tree.jetTrim1_aplanarity - means['aplanarity'])/ std['aplanarity']
            dnn_predict_arr_2['split12'][0] = (tree.jetTrim1_groosplit12 - means['split12']) / std['split12']
            dnn_predict_arr_2['tauwta2tauwta1'][0] = (tree.jetTrim1_grootau21 - means['tauwta2tauwta1']) / std['tauwta2tauwta1']
            dnn_predict_arr_2['planarflow'][0] = (tree.jetTrim1_planarflow - means['planarflow']) / std['planarflow']
            dnn_predict_arr_2['sphericity'][0] = (tree.jetTrim1_sphericity - means['sphericity']) / std['sphericity']
            
            j1_dnn_pred = dnn_model.predict(dnn_predict_arr)[0]
            j2_dnn_pred = dnn_model.predict(dnn_predict_arr_2)[0]
            jetTrim1_bdt[0] = j1_dnn_pred[0]
            jetTrim2_bdt[0] = j2_dnn_pred[0]
...
def analyse(bdt_model, bdt_taggers, dnn_model, dnn_taggers, dnn_scaler, data_files):
    ...
    for data_file in data_files:
        ...
        bdt_proba = bdt_model.predict_proba(bdt_data)
        bdt_proba_2 = bdt_model.predict_proba(bdt_data_2)
        # scale data
        ...
        data_scored = nf.append_fields(data_arr, names=['jetTrim1_bdt','jetTrim1_dnn','jetTrim2_bdt','jetTrim2_dnn'], data=[bdt_proba, bdt_proba_2, dnn_predictions, dnn_prediction_2], usemask = False)
        array2root(data_scored, data_file.replace('.root','_scored.root'),'dibjet','recreate')
...
def main(args):
    ...
    # load the dnn schema
    dnn_file_obj = open(args.data_file,'r')
    schema = yaml.load(dnn_file_obj)
    dnn_model = apy.NeuralNet()
    dnn_tagger_file, specifications = schema['taggers']
    ...
    analyse(bdt_model, bdt_taggers, dnn_model, dnn_taggers, dnn_scaler, args.data_input_file)
